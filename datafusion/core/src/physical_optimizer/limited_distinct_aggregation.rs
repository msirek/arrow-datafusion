...
impl LimitedDistinctAggregation {
    ...
    /// and pushes the limit into the aggregation as a soft limit when there is a group by,
    /// but no sorting, no aggregate expressions, and no filters in the aggregation
    fn transform_local_limit(
        plan: Arc<dyn ExecutionPlan>,
    ) -> Option<Arc<dyn ExecutionPlan>> {
        let limit = plan.as_any().downcast_ref::<LocalLimitExec>()?;
        ...
        if limit.output_ordering().is_some() {
            return None;
        }
        // ensure no ordering is required on the input
        if limit.required_input_ordering()[0].is_some() {
            return None;
        }

        // if found_final_aggr is true, final_aggr holds a final aggregation whose group_by
        // must match that of the partial aggregation in order to rewrite the partial aggregation
        let mut final_aggr: Arc<dyn ExecutionPlan> = plan;
        let mut found_final_aggr = false;
        ...
        let mut closure = |plan: Arc<dyn ExecutionPlan>| {
            ...
            if let Some(aggr) = plan.as_any().downcast_ref::<AggregateExec>() {
                ...
                match Self::transform_agg(aggr, fetch) {
                    None => {}
                    Some(new_aggr) => {
                        if matches!(
                            aggr.mode(),
                            ...
                            final_aggr = plan;
                            found_final_aggr = true;
                        } else {
                            // for non-final flavors of aggregation, don't apply the rewrite
                            // to a child partial aggregation
                            rewrite_applicable = false;
                        }
        ...
        Some(Arc::new(LocalLimitExec::new(child, fetch)))
    }
}
...
mod tests {
    ...
    #[test]
    fn test_partial_final() -> Result<()> {
        let source = mock_data()?;
        let schema = source.schema();

        // `SELECT a FROM MemoryExec GROUP BY a LIMIT 10;`, Partial/Final AggregateExec
        ...
        let expected = [
            ...
            "AggregateExec: mode=Final, gby=[a@0 as a], aggr=[], lim=[10]",
            "AggregateExec: mode=Partial, gby=[a@0 as a], aggr=[], lim=[10]",
            "MemoryExec: partitions=1, partition_sizes=[1]",
        ];
        assert_plan_matches_expected(limit_exec, &expected)?;
        Ok(())
    }
    ...
    fn test_single() -> Result<()> {
        ...
        // `SELECT a FROM MemoryExec GROUP BY a LIMIT 10;`, Single AggregateExec
        ...
        let expected = [
            "LocalLimitExec: fetch=10",
            "AggregateExec: mode=Single, gby=[a@0 as a], aggr=[], lim=[10]",
            "MemoryExec: partitions=1, partition_sizes=[1]",
        ];
        assert_plan_matches_expected(limit_exec, &expected)?;
        Ok(())
    ...
    #[test]
    fn test_distinct_cols_different_than_group_by_cols() -> Result<()> {
        let source = mock_data()?;
        let schema = source.schema();

        // `SELECT distinct a FROM MemoryExec GROUP BY a, b LIMIT 10;`, Single/Single AggregateExec
